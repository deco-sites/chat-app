...
const Home = ({ pageTitle, posts }: Props) => {
  return (
    <div className="w-full min-h-screen flex flex-col bg-gray-200">
      <Header pageTitle={pageTitle ?? "Mais recentes"} />
      <section className="max-w-[1180px] min-h-full flex flex-col justify-center mx-auto py-10 px-3 gap-5">
            ...
            <div className="flex h-full w-full flex-col gap-5 justify-center md:justify-start items-center py-4 bg-gray-100 p-3 rounded-md">
              <div className="flex h-[10.5rem] w-full justify-center gap-4 items-center pb-10">
                <div className="h-[7.8rem] w-[12rem] flex flex-col justify-center items-center p-2 gap-2">
                  ...
                  <span className="text-[#292929] font-sans text-center text-lg">
                    {post.user.name}
                  </span>
                </div>
                ...
                <div className="w-full flex flex-col justify-center md:gap-2">
                  <span className="w-full h-full flex flex-col md:flex-row items-center gap-2">
                    <div className="rating rating-sm">
                      <input
                        type="radio"
                        name={`rating-${post.rating}`}
                        className={`mask mask-star-2 ${
                          post.rating >= 1 ? "bg-orange-400" : "bg-gray-400"
                        }`}
                      />
                      <input
                        type="radio"
                        name={`rating-${post.rating}`}
                        className={`mask mask-star-2 ${
                          post.rating >= 2 ? "bg-orange-400" : "bg-gray-400"
                        }`}
                      />
                      <input
                        type="radio"
                        name={`rating-${post.rating}`}
                        className={`mask mask-star-2 ${
                          post.rating >= 3 ? "bg-orange-400" : "bg-gray-400"
                        }`}
                      />
                      <input
                        type="radio"
                        name={`rating-${post.rating}`}
                        className={`mask mask-star-2 ${
                          post.rating >= 4 ? "bg-orange-400" : "bg-gray-400"
                        }`}
                      />
                      <input
                        type="radio"
                        name={`rating-${post.rating}`}
                        className={`mask mask-star-2 ${
                          post.rating >= 5 ? "bg-orange-400" : "bg-gray-400"
                        }`}
                      />
                    </div>
                    <span className="text-[#787878] font-normal font-sans">
                      • {post.releaseDate ?? "2 weeks ago"}
                    </span>
                  </span>

                  <div className="flex gap-3 flex-col">
                    <span className="text-xs font-sans md:text-normal">
                      {post.title}
                    </span>
                    <span className="text-xs font-sans md:text-lg">
                      {post.content}
                    </span>
                  </div>
                </div>
        ...
        <div className="flex justify-center items-center space-x-1">
          <button
            className={
              "flex items-center px-4 py-2 font-sans font-normal text-gray-500 rounded-md hover:bg-slate-700 bg-slate-800 hover:text-white"
            }
          >
            Anterior
          </button>
          ...
          <button
            className={
              "flex items-center font-sans px-4 py-2 font-normal rounded-md hover:bg-slate-700 text-gray-100 bg-slate-800 hover:text-white"
            }
          >
            Próximo
          </button>
